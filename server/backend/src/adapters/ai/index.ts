...
async function requestOpenAiJson(system: string, user: string): Promise<unknown> {
  ...
  try {
    const response = await fetch(config.aiApiUrl, {
      ...
      body: JSON.stringify({
        ...
        response_format: { type: "json_schema", json_schema: { name: "civicx_classification", strict: true, schema: classificationJsonSchema } },
