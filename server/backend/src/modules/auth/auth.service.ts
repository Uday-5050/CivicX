...
export async function verifyAccessToken(token: string): Promise<{ userId: string; role: Role }> { const { payload } = await jwtVerify(token, accessSecret, { algorithms: ["HS256"] }); if (typeof payload.sub !== "string" || !["citizen", "university", "industry", "admin", "government"].includes(String(payload.role))) throw new Error("Invalid access token"); return { userId: payload.sub, role: payload.role as Role }; }
