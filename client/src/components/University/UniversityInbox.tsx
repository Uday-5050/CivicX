...
export default function UniversityInbox() {
  ...
  const load = async () => { const next = await listUniversityChallenges(); setChallenges(next); setSelected((current) => current ? next.find((item) => item.id === current.id) ?? next[0] : next[0]) }
  useEffect(() => { void load() }, [])
  ...
  const decide = async (decision: Exclude<ChallengeDecision, 'pending'>) => {
    ...
    try {
      ...
      setChallenges((current) => current.map((item) => item.id === updated.id ? updated : item)); setSelected(updated); setProposalOpen(false); setNotice(decision === 'accepted' ? 'Project created and proposal saved.' : decision === 'declined' ? 'Challenge declined.' : 'Request for more information sent.')
