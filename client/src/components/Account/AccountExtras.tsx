...
export default function AccountExtras() {
  ...
  useEffect(() => { if (typeof Notification === 'undefined') setPushState('unsupported'); else setPushState(Notification.permission); if (user?.role === 'university') void listInstitutionMembers().then(setMembers).catch(() => setNotice('Institution member management is not available yet.')) }, [user?.id, user?.role])
  if (!user) return null
  ...
  const updateMember = async (member: InstitutionMember) => { setBusy(true); try { const updated = await setInstitutionMemberStatus(member.id, member.status === 'active' ? 'suspended' : 'active'); setMembers((current) => current.map((item) => item.id === updated.id ? updated : item)); setNotice(`${updated.name} is now ${updated.status}.`) } catch { setNotice('Institution member management is not available yet.') } finally { setBusy(false) } }
