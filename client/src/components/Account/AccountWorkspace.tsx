...
export default function AccountWorkspace() {
  ...
  const readOne = async (notification: CivicNotification) => { if (notification.read) return; try { const updated = await markNotificationRead(notification.id); setNotifications((current) => current.map((item) => item.id === updated.id ? updated : item)) } catch { setNotice('That notification is not available to this account.') } }
