...
export default function AccountWorkspace() {
  ...
  const loadNotifications = async () => { if (!user) return; try { setNotifications(await listNotifications()); setNotice('') } catch { setNotifications([]); setNotice('Notifications are not available yet.') } }
  useEffect(() => {
    if (!user) return
    ...
    const poll = window.setInterval(() => { if (document.visibilityState === 'visible') void loadNotifications() }, 30000)
    ...
    return () => { window.clearInterval(poll); window.removeEventListener('online', reconnect); document.removeEventListener('visibilitychange', reconnect); window.removeEventListener('notification:new', notificationEvent) }
  }, [user?.id])
