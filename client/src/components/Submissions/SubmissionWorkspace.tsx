...
export default function SubmissionWorkspace({ role }: { role: Role }) {
  ...
  useEffect(() => {
    void listSubmissions().then(setSubmissions).catch((error: unknown) => setError(submissionError(error)))
    try {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      localStorage.setItem(draftKey, JSON.stringify({ form, locationData, attachmentNames: attachments.map((attachment) => attachment.name), savedAt } satisfies SavedDraft))
      setDraftSavedAt(savedAt)
    }, 400)
    return () => window.clearTimeout(timer)
  }, [draftReady, form, locationData, attachments])
