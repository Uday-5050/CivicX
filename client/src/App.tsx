...
function PasswordReset({ reset = false }: { reset?: boolean }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (reset) {
        await request('/auth/reset-password', { method: 'POST', data: { token, password } })
      } else {
        await request('/auth/forgot-password', { method: 'POST', data: { email } })
      }
      setSent(true)
    } catch (err) {
...
function UniversityPage() {
  const { user, logout } = useAuth()
  useEffect(() => { if (!user || user.role !== ROLES.UNIVERSITY) go('/login') }, [user])
  if (!user || user.role !== ROLES.UNIVERSITY) return null
...
function IndustryPage() {
  const { user, logout } = useAuth()
  useEffect(() => { if (!user || user.role !== ROLES.INDUSTRY) go('/login') }, [user])
...
function ProjectBoardPage() {
  const { user, logout } = useAuth()
  useEffect(() => { if (!user || (user.role !== ROLES.UNIVERSITY && user.role !== ROLES.INDUSTRY)) go('/login') }, [user])
  if (!user || (user.role !== ROLES.UNIVERSITY && user.role !== ROLES.INDUSTRY)) return null
...
function GovernmentLogin() {
  ...
  return <AuthLayout eyebrow="Government portal" title="Monitor. Measure. Transform." copy="Access the Jharkhand Innovation Command Centre to track societal challenges, university engagement, industry collaboration, and measurable outcomes across all 24 districts."><form className="auth-form" onSubmit={submit}><label>Government email<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="username" required /></label><label>Password<div className="auth-password-wrap"><input type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Enter government credentials" autoComplete="current-password" required /><button type="button" className="auth-password-toggle" onClick={() => setShowPassword((v) => !v)} aria-label={showPassword ? 'Hide password' : 'Show password'}>{showPassword ? '◎' : '◉'}</button></div></label>{error && <p className="inline-error" role="alert">{error}</p>}<button className="auth-submit" type="submit" disabled={busy}>{busy ? 'Signing in...' : 'Open government dashboard'} <span>→</span></button><p className="form-foot">Government accounts are provided by the platform administrator. <a href="#/">Return to CivicX</a></p></form></AuthLayout>
}
...
function Landing() {
  ...
  return <main className="landing-page"><div className="landing-canvas" aria-hidden="true"><Threads color={[0.38, 0.72, 1]} amplitude={1.15} distance={0.12} enableMouseInteraction /></div><div className="landing-wash" aria-hidden="true" /><nav className="site-nav" aria-label="Main navigation"><Brand /><div className="nav-links"><button type="button" className="nav-anchor" onClick={() => scrollTo('services')} style={{ background: 'none', border: 'none', color: 'inherit', font: 'inherit', cursor: 'pointer' }}>Services</button><button type="button" className="nav-anchor" onClick={() => scrollTo('about')} style={{ background: 'none', border: 'none', color: 'inherit', font: 'inherit', cursor: 'pointer' }}>About CivicX</button><a href="#/government">Government</a></div><a className="nav-login" href="#/login">Sign in <span>↗</span></a></nav><section className="hero-copy" id="about"><p className="eyebrow"><span /> समस्या से समाधान तक</p><h1>From problems to solutions,<br /><em>CivicX is with you.</em></h1><p className="hero-description">Access public services, follow your requests, and build a better community from one trusted place.</p><div className="hero-actions"><a className="btn btn-primary" href="#/register">Get started <span>→</span></a><a className="text-link" href="#/login">Already registered? <strong>Sign in</strong></a></div></section><section className="service-bar" id="services" aria-label="CivicX services"><div className="service-intro"><span className="live-dot" /> Your civic space</div><div className="service-item"><span>01</span><strong>Raise a request</strong><small>Be heard, be counted</small></div><div className="service-item"><span>02</span><strong>Track progress</strong><small>Stay in the loop</small></div><div className="service-item"><span>03</span><strong>Shape tomorrow</strong><small>Take part locally</small></div></section></main>
}
